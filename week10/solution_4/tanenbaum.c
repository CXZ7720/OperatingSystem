...
void test(int phil) {
  if (state[phil] == HUNGRY && state[LEFT] != EATING &&
    ...
    sem_post(&forks[phil]); //DONE
  }
}

void take_forks(int phil) {
  /* TODO 4: Wait for the waiter */
  pthread_mutex_lock(&mutex); //DONE
...
int main() {
  ...
  pthread_mutex_init(&mutex, NULL); //DONE

  for (i = 0; i < N; i++) {
    /* TODO 10: Initialize each forks with 0 */
    sem_init(&forks[i], 0,
             0); // Assign semaphore as a binary value (0 or 1) (== mutex)
  }
